...
```vue
<template>
  <GanttChartComponent :date-range="dateRange" :rows="rows" header-label="gantt.header" />
</template>
...
const dateRange = ref([new Date(2026, 0, 1), new Date(2026, 0, 2)])
const rows = ref([
  {
    id: 1,
    header: 'gantt.row.lineA',
    activities: [
      {
        id: 'planned',
        label: 'gantt.activity.planned',
      ...
      {
        id: 'optimized',
        label: 'gantt.activity.optimized',
...
- Week labels use the key `Week` by default.
