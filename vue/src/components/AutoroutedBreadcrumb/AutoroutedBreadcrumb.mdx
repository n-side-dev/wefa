...
For the breadcrumb to display meaningful information, your routes should include `meta.wefa` metadata with `title` and optionally `icon`:
...
3. Transforming each matched route into a breadcrumb item with:
   - Label: Uses `route.meta.wefa.title`, or falls back to the route name
   - Icon: Uses `route.meta.wefa.icon` if available
...
- If a route doesn't have a `meta.wefa.title` property, the component will fall back to using the route name
