...
The translation feature is powered by `vue-i18n` and configured in `src/locales/index.ts`.

`createLibI18n` bundles three layers of translations:

1. **wefa defaults** — wefa's own English messages for its components (form labels, validation, breadcrumbs, etc.) are inlined into the library bundle at build time.
2. **PrimeVue locale** — translations from [`primelocale`](https://github.com/primefaces/primelocale) for all supported languages. The plugin installs a watcher that syncs PrimeVue's `config.locale` with the active vue-i18n locale, so switching locale at runtime updates PrimeVue components (DatePicker day names, DataTable messages, etc.) automatically.
3. **Project translations** — the consumer's own JSON files under `src/locales/<locale>/<category>.json`, passed via the `glob` option. These override wefa's defaults on a per-key basis.
...
import { createLibI18n } from '@nside/wefa'
import App from './App.vue'

const app = createApp(App)

// PrimeVue MUST be installed before createLibI18n's result so the i18n plugin
// can sync PrimeVue's `config.locale`.
app.use(PrimeVue, {
  theme: {
    /* ... */
  },
...
app.use(
  createLibI18n({
    glob: import.meta.glob('@/locales/**/*.json', { eager: true }),
    locale: 'en',
  })
)
...
Drop your project's JSON files under `src/locales/<locale>/<category>.json`; they'll be merged in automatically.
...
- **Note**: Ensure `route.meta.wefa.title` contains valid keys (e.g., `breadcrumb.home`) from your translation files.
