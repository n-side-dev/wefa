  ...
  <div>
    ...
    >
      ...
      <span v-if="mode === 'detailed'" class="grow truncate text-left">
        <span class="block truncate text-sm/5 font-medium text-[var(--brand-text-on-dark)]">
          {{ username }}
        </span>
        <span class="block truncate text-sm/5 font-normal text-[var(--brand-text-on-dark-muted)]">
          {{ email }}
        </span>
      </span>
      <i
        v-if="mode === 'detailed'"
        class="pi pi-chevron-up text-lg text-[var(--brand-text-on-dark-subtle)]"
      ></i>
    </button>
...
const triggerClass = computed(() => {
  if (mode === 'compact') {
    return [
      ...
      'hover:bg-[var(--brand-nav-hover-bg)]',
    ]
  }

  return [
    'flex w-full cursor-pointer items-center gap-3 rounded-[1.25rem] border border-[var(--brand-nav-card-border)] bg-[var(--brand-nav-card-bg)] px-3 py-3 text-[var(--brand-text-on-dark)] transition',
    'hover:bg-[var(--brand-nav-hover-bg)]',
  ]
})
