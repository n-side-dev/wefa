...
## BFF authentication (Keycloak via proxy)

When using the BFF authentication mode, all HTTP calls must include credentials so the session cookie is sent.
The backend store configures the axios singleton with the fetch adapter and CORS credentials:
...
- `backendStore.login()` calls the login endpoint. If `loginRedirect` is enabled, it redirects to the URL returned by the BFF.
- `backendStore.logout()` calls the logout endpoint and clears the auth state.
- `backendStore.checkSession()` calls the session endpoint and updates `authenticated`.
- `backendStore.fetchUserInfo()` fetches the user profile and updates `authenticated`.

The default login/logout views in the library consume the same `bff.flow` configuration to decide whether to auto-run and redirect.

### Buttons for layout usage

If you do not want dedicated login/logout pages, you can drop buttons into your layout. These buttons adapt to the configured authentication mode:

```vue
<script setup lang="ts">
import { LoginButton, LogoutButton } from '@nside/wefa'
</script>
...
To automatically trigger login when a user reaches a protected route while unauthenticated, install the plugin:

```ts
import router from '@/router'
import { bffAuthPlugin } from '@nside/wefa'

app.use(bffAuthPlugin, { router, backendStore })
```

This calls `checkSession()` and, if needed, starts the login flow.

### Quick demo test

1. Update `vue/src/demo/backendStore.ts` to use `authenticationType: 'BFF'` and the BFF options above.
2. Add the auth routes in `vue/src/demo/router.ts` using `libRouteRecords.login` and `libRouteRecords.logout`.
3. Run `npm run dev` and visit `/auth/login` to trigger the flow.
...
const {
  /* ... */
} = apiClient.query(ref('/api/my/url'))
const {
  /* ... */
} = apiClient.mutation(ref('/api/my/url'), myBody)

const {
  /* ... */
} = typedApiClient.query(getHello, options)
const {
  /* ... */
} = typedApiClient.mutation(postHello, options)
```
