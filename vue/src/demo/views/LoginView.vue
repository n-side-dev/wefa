<template>
  ...
  >
    ...
    <div class="relative z-10 flex w-full max-w-[33rem] flex-col items-center">
      <img :src="logoLight" alt="N-SIDE" class="mb-8 h-auto w-[15rem] dark:hidden" />
      <img :src="logoDark" alt="N-SIDE" class="mb-8 hidden h-auto w-[15rem] dark:block" />
...
import logoDark from '@/demo/assets/nside-logo-white.png'
import logoLight from '@/demo/assets/nside-logo.png'
...
/**
 * Submit the login form against the backend store, handling redirect and errors.
 */
async function handleSubmit() {
  submitting.value = true
  try {
    await backendStore.login({ username: username.value, password: password.value })
    const redirect = (router.currentRoute.value.query.redirect as string | undefined) ?? '/home'
    await router.push(redirect)
  } catch (error) {
    console.error('[demo] login failed:', error)
    toast.add({
      severity: 'error',
      summary: t('demo.login.error_summary'),
      detail: t('demo.login.error_detail'),
      life: 5000,
    })
  } finally {
...
<style scoped>
:deep(.nside-password .p-inputtext) {
  width: 100%;
}

:deep(.nside-password .p-password-toggle-mask-icon) {
  right: 1rem;
