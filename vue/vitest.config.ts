...
function supportsStorybookVitestPlugin() {
  const [major, minor] = process.versions.node.split('.').map((segment) => Number(segment))
  return major > 20 || (major === 20 && minor >= 6)
}

function resolveStorybookProject():
  | NonNullable<NonNullable<UserConfig['test']>['projects']>[number]
  | null {
  if (!supportsStorybookVitestPlugin()) {
    return null
  }

  const { storybookTest } =
    require('@storybook/addon-vitest/vitest-plugin') as typeof import('@storybook/addon-vitest/vitest-plugin')
...
const storybookProject = resolveStorybookProject()
...
const vitestConfig: UserConfig = {
  test: {
    ...
    coverage: {
      ...
      exclude: [...coverageConfigDefaults.exclude, 'dist-demo/**', 'src/main.ts', 'src/demo/**'],
    },
    projects: [
